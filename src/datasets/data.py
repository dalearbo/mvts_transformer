...
boto3.setup_default_session(profile_name="upbeat")
...
class UpbeatData(BaseData):
    ...
    def __init__(
        ...
        self.all_df = self.all_df.sort_values(
            by=["event_id", "date_offset"]
        )  # datasets is presorted
        self.all_df = self.all_df.set_index("event_id")
        self.all_IDs = self.all_df.index.unique()  # all sample (session) IDs
        self.max_seq_len = 365
        ...
        self.label_names = ["25", "50", "100", "250", "500", "501"]
        self.feature_df = self.all_df[self.feature_names]
        self.labels_df = self.all_df[self.label_names]
    ...
    def load_all(self, root_dir, file_list=None, pattern=None):
        ...
        bucket = s3.Bucket("upbeat-data")
        data_paths = [
            item.key
            for item in bucket.objects.filter(Prefix="ml-training-data/2022-05-14")
        ]
        if len(data_paths) == 0:
            raise Exception(
        ...
        input_paths = [p for p in selected_paths if p.endswith(".csv")]
        ...
        if self.n_proc > 1:
            ...
            with Pool(processes=_n_proc) as pool:
                all_df = pd.concat(pool.map(UpbeatData.load_single, input_paths))
        else:  # read 1 file at a time
            all_df = pd.concat(UpbeatData.load_single(path) for path in input_paths)

        return all_df

    @staticmethod
    def load_single(filepath):
        df = UpbeatData.read_data(filepath)
        df = UpbeatData.select_columns(df)
        df["population"] = df[["city_pop", "population"]].max(axis=1)
        df = df.drop("city_pop", axis=1)
        output_df = pd.DataFrame()
        for event_id in pd.unique(df.event_id):
            df_event = df[df.event_id == event_id].set_index("date_offset")
            blank_df = pd.DataFrame(index=list(range(0, 365)))
            df_event = blank_df.join(df_event)
            for column_name in ["listeners", "population", "score"]:
                df_event[column_name] = df_event[column_name].interpolate()
            df_event[pd.isnull(df_event["event_id"])] = -1
            df_event = df_event.fillna(value=-2).drop("event_id", axis=1).reset_index()
            output_df = pd.concat((output_df, df_event))
        print(output_df)
    ...
    def read_data(filepath):
        ...
        # return pd.read_csv("2022-08-27.csv")
        s3 = boto3.client("s3")
        try:
            df = pd.read_csv(
                s3.get_object(Bucket="upbeat-data", Key=filepath).get("Body")
            )
            return df
        except:
            print(f"Error loading key {filepath}")
            return pd.DataFrame()

    @staticmethod
    def select_columns(df):
        df["date_offset"] = df["date_offset"].astype(int)
        keep_cols = [
            "event_id",
            "date_offset",
            "day_of_week",
